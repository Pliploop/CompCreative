...
class TextAudioDataset(Dataset):
    def __init__(self,
        ...
        #keep only caption, file_path
        # annotations = [{'caption': annot['caption'], 'file_path': annot['file_path']} for annot in annotations]
        ...
        # back to records
        # annotations = annot_df.to_dict('records')
        ...
        for i in range(len(self)):
            try:
                ...
                if hasattr(model,'get_text_embedding'):
                    text_features = model.get_text_embedding(text)
                    ## sanity check similarity
                    audio_features_ = audio_features.mean(0,keepdim=True)
                    sims = audio_features_ @ text_features['projected_pooler_output'].t()
                    print(f"Similarity between audio and text: {sims.item()}")
